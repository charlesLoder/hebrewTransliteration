...
test.describe("/transliterate", () => {
  ...
  test.describe("ON_COMPLETE Callback", () => {
    test.beforeEach(async ({ page }) => {
      await page.goto("/");
      await page.waitForLoadState("networkidle");
      await page.evaluate(() => localStorage.clear());
    ...
    test("toggle enables editor and code persists after reload", async ({ page }) => {
      const settingsBtn = page.getByTestId("settings-button");
      await settingsBtn.click();
      await page.getByRole("tab", { name: "Others" }).click();

      const toggle = page.locator("#enable-on-complete");
      await toggle.check();

      const editor = page.getByTestId("on-complete-editor");
      await expect(editor).toBeVisible({ timeout: 5000 });

      const cm = editor.locator(".cm-content");
      await cm.click();
      await page.keyboard.press("ControlOrMeta+A");
      await page.keyboard.type("(result, context) => { return result + ' [processed]'; }");
      ...
      await expect(cm2).toContainText("[processed]");
    });

    test("disabling toggle removes ON_COMPLETE from schema", async ({ page }) => {
      const settingsBtn = page.getByTestId("settings-button");
      await settingsBtn.click();
      await page.getByRole("tab", { name: "Others" }).click();

      await page.locator("#enable-on-complete").check();
      await expect(page.getByTestId("on-complete-editor")).toBeVisible({ timeout: 5000 });

      await page.locator("#enable-on-complete").uncheck();
      await expect(page.getByTestId("on-complete-editor")).not.toBeVisible();
    });
  });

  test.describe("Ketiv/Qere Pairs", () => {
    test.beforeEach(async ({ page }) => {
      await page.goto("/");
      await page.waitForLoadState("networkidle");
      await page.evaluate(() => localStorage.clear());
    ...
    async function open_syllabification_tab(page: import("@playwright/test").Page) {
      await page.getByTestId("settings-button").click();
      await page.getByRole("tab", { name: "Syllabification" }).click();
    }

    test("pair does not disappear when typing input", async ({ page }) => {
      await open_syllabification_tab(page);

      await page.getByRole("button", { name: "+ Add Pair" }).click();

      const input = page.locator("#kq-input-0");
      await expect(input).toBeVisible({ timeout: 5000 });
      await input.fill("יהוה");

      await expect(page.locator("#kq-input-0")).toHaveValue("יהוה");
      await expect(page.locator("#kq-output-0")).toBeVisible();
    });

    test("output field does not echo input", async ({ page }) => {
      await open_syllabification_tab(page);
      ...
      const output = page.locator("#kq-output-0");
      await expect(output).toHaveValue("");
    });

    test("string output is committed correctly", async ({ page }) => {
      await open_syllabification_tab(page);

      await page.getByRole("button", { name: "+ Add Pair" }).click();

      await page.locator("#kq-input-0").fill("שלום");
      await page.locator("#kq-output-0").fill("shalom");
      await page.locator("#kq-input-0").blur();

      await expect(page.locator("#kq-output-0")).toHaveValue("shalom");
    });

    test("round-trip persists pairs through reload", async ({ page }) => {
      await open_syllabification_tab(page);

      await page.getByRole("button", { name: "+ Add Pair" }).click();
      await page.locator("#kq-input-0").fill("אלהים");
      await page.locator("#kq-output-0").fill("elohim");
      await page.locator("#kq-input-0").blur();
      ...
      await expect(page.locator("#kq-input-0")).toHaveValue("אלהים");
      await expect(page.locator("#kq-output-0")).toHaveValue("elohim");
    });

    test("callback output via KetivQereEditor", async ({ page }) => {
      await open_syllabification_tab(page);

      await page.getByRole("button", { name: "+ Add Pair" }).click();
      await page.locator("#kq-input-0").fill("test");

      await page.locator("#kq-out-callback-0").click();

      await page.getByRole("button", { name: "Edit Callback" }).click();

      const cm = page.getByTestId("kq-callback-editor").locator(".cm-content");
      await expect(cm).toBeVisible({ timeout: 5000 });
      await cm.click();
      await page.keyboard.press("ControlOrMeta+A");
      await page.keyboard.type("(text, input) => { return text.replace(input, 'REPLACED'); }");

      await page.waitForTimeout(500);

      await page.getByTestId("kq-save-button").click();

      await expect(page.getByText("(callback)")).toBeVisible();
    });

    test("callback survives reload round-trip", async ({ page }) => {
      await open_syllabification_tab(page);

      await page.getByRole("button", { name: "+ Add Pair" }).click();
      await page.locator("#kq-input-0").fill("/hello/");
      ...
      await cm.click();
      await page.keyboard.press("ControlOrMeta+A");
      await page.keyboard.type("(match, input) => match.toUpperCase()");
      ...
      const cm2 = page.getByTestId("kq-callback-editor").locator(".cm-content");
      await expect(cm2).toBeVisible({ timeout: 5000 });
      await expect(cm2).toContainText("toUpperCase");
    });

    test("ignoreTaamim and captureTaamim checkboxes toggle correctly", async ({ page }) => {
      await open_syllabification_tab(page);

      await page.getByRole("button", { name: "+ Add Pair" }).click();
      await page.locator("#kq-input-0").fill("test");

      const ignoreBox = page.locator("#kq-ignore-0");
      const captureBox = page.locator("#kq-capture-0");

      await expect(ignoreBox).toBeChecked();
      await expect(captureBox).not.toBeChecked();

      await ignoreBox.uncheck();
      await captureBox.check();

      await expect(ignoreBox).not.toBeChecked();
      await expect(captureBox).toBeChecked();
    });

    test("deleting a pair removes it from the list", async ({ page }) => {
      await open_syllabification_tab(page);

      await page.getByRole("button", { name: "+ Add Pair" }).click();
      await expect(page.locator("#kq-input-0")).toBeVisible();

      await page.getByRole("button", { name: "Delete" }).click();
      await expect(page.locator("#kq-input-0")).not.toBeVisible();
    });

    test("regex input is preserved after reload", async ({ page }) => {
      await open_syllabification_tab(page);

      await page.getByRole("button", { name: "+ Add Pair" }).click();
      await page.locator("#kq-input-0").fill("/[\\u{05B4}]/u");
      await page.locator("#kq-output-0").fill("x");
      await page.locator("#kq-input-0").blur();
      ...
      await expect(page.locator("#kq-input-0")).toHaveValue("/[\\u{05B4}]/u");
    });
  });
});
